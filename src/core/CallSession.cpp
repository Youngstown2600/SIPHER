...
void CallSession::setQueueAudioFile(std::string path)
{
    {
        std::lock_guard<std::mutex> lock(mutex_);
        queueAudioPath_=std::move(path);
    }
    attachQueueAudio();
}

void CallSession::attachQueueAudio()
{
    std::string path;
    CallPurpose purpose;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        path=queueAudioPath_;
        purpose=snapshot_.purpose;
        if(path.empty()||purpose!=CallPurpose::QueueTest||queueAudioAttached_)return;
    }
    try{
        const auto info=getInfo();
        for(unsigned i=0;i<info.media.size();++i){
            if(info.media[i].type!=PJMEDIA_TYPE_AUDIO||info.media[i].status!=PJSUA_CALL_MEDIA_ACTIVE)continue;
            if(!queueAudioPlayer_){
                queueAudioPlayer_=std::make_unique<pj::AudioMediaPlayer>();
                queueAudioPlayer_->createPlayer(path);
            }
            auto media=getAudioMedia(static_cast<int>(i));
            queueAudioPlayer_->startTransmit(media);
            {std::lock_guard<std::mutex> lock(mutex_);queueAudioAttached_=true;}
            logger_.info("Queue audio attached to call "+std::to_string(getId())+": "+path);
            break;
        }
    }catch(const pj::Error&e){logger_.warn("Unable to attach queue audio: "+e.info());}
}

void CallSession::detachQueueAudio()
{
    if(!queueAudioPlayer_)return;
    try{
        const auto info=getInfo();
        for(unsigned i=0;i<info.media.size();++i){
            if(info.media[i].type!=PJMEDIA_TYPE_AUDIO)continue;
            try{auto media=getAudioMedia(static_cast<int>(i));queueAudioPlayer_->stopTransmit(media);}catch(...){}
        }
    }catch(...){}
    queueAudioPlayer_.reset();
    std::lock_guard<std::mutex> lock(mutex_);queueAudioAttached_=false;
}
...
void CallSession::detachAudio()
{
    ...
    // to the headset. Background queue calls can have healthy active RTP.
}
